...
@interface SetCBT : GHAsyncTestCase
@end
...
- (void)test_SET_CBT_001 {
    ...
    GHAssertFalse([pYuMeSDK yumeSdkSetControlBarToggle:TRUE errorInfo:&pError], @"SetControlBarToggle Successful.");
    if (pError) {
        NSString *str = [YuMeUnitTestUtils getErrDesc:pError];
        GHAssertEqualStrings(str, @"yumeSdkSetControlBarToggle(): YuMe SDK is not Initialized.", @"yumeSdkSetControlBarToggle(): YuMe SDK is not Initialized.");
...
- (void)test_SET_CBT_002 {
    ...
    GHTestLog(@"Default bEnableCBToggle value : %d", testCBToggle);

    GHAssertTrue([pYuMeSDK yumeSdkSetControlBarToggle:(testCBToggle == TRUE ? FALSE : TRUE) errorInfo:&pError], @"SetControlBarToggle Successful.");
    if (pError) {
        NSString *str = [YuMeUnitTestUtils getErrDesc:pError];
        GHFail(@"Result : %@", str);
    }
    ...
    GHTestLog(@"After modified bEnableCBToggle value : %d", adParams.bEnableCBToggle);
    
    GHAssertNotEquals(testCBToggle, adParams.bEnableCBToggle, @"Sets the new value for control bar toggle flag in YuMeAdParams object stored in the SDK.");
}

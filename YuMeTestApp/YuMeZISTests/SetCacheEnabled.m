...
@interface SetCacheEnabled : XCTAsyncTestCase
@end
...
- (void)test_SET_CE_001 {
    ...
    XCTAssertFalse([pYuMeSDK yumeSdkSetCacheEnabled:TRUE errorInfo:&pError], @"yumeSdkSetCacheEnabled Successful.");
    if (pError) {
        NSString *str = [YuMeUnitTestUtils getErrDesc:pError];
        XCTAssertEqualObjects(str, @"yumeSdkSetCacheEnabled(): YuMe SDK is not Initialized.", @"yumeSdkSetCacheEnabled(): YuMe SDK is not Initialized.");
        NSLog(@"Result : %@", str);
    }
}
...
- (void)test_SET_CE_002 {
    ...
    NSLog(@"Default bEnableCaching value : %d", bEnableCaching);
    
    XCTAssertTrue([pYuMeSDK yumeSdkSetCacheEnabled:(bEnableCaching == TRUE ? FALSE : TRUE) errorInfo:&pError], @"SetCacheEnabled Successful.");
    if (pError) {
        NSString *str = [YuMeUnitTestUtils getErrDesc:pError];
        XCTFail(@"%@", str);
    }
    ...
    NSLog(@"After modified bEnableCaching value : %d", adParams.bEnableCaching);
    
    XCTAssertNotEqual(bEnableCaching, adParams.bEnableCaching, @"Updates the value in its local cache based on the result of JS SDK Call.");
}
